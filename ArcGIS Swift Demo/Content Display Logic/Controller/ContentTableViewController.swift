...
class ContentTableViewController: UITableViewController, CustomSearchHeaderViewDelegate, DownloadProgressViewDelegate {
    ...
    private lazy var __once: () = { [weak self] in //321了解一下lazy加载
            self?.animateTable()
    }()
    ...
    override func viewDidLoad() {
        ...
        if containsSearchResults {//321 没明白啥意思 有搜索结果的话移除headview? 没有就加一个？
    ...
    override func viewWillAppear(_ animated: Bool) {//321啥意思 这用法?
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if node.dependency.count > 0 { //321下载资源了解一下
    ...
    func downloadResource(for node:Node, at indexPath:IndexPath) {
        ...
        self.bundleResourceRequest.beginAccessingResources { [weak self] (error: Error?) in
            ...
            DispatchQueue.main.sync {
                ...
                //remove observer  //321添加了observer后怎么又删除了 不应该是成功后才删除吗
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) { //321这个方法干嘛用的 是观察属性变化 小数完成就成功 所以上面下载的方法中可以移除吗 不太明白
